...
exports.GroqFallbackDetector = void 0;
...
`;
class GroqFallbackDetector extends BaseDetector_1.BaseDetector {
    apiKey;
    model;
    maxEvents;
    timeout;
    ...
    async detect(entry) {
        ...
        const eventText = sourceEvents
            ...
            .join('\n');
        if (eventText)
            this.pendingEntries.push({ ref: entry.externalRef ?? 'unknown', text: eventText });
        return [];
    }
    async finalize() {
    ...
    async callGroqWithRetry(text, ref, retries) {
    ...
    async callGroqAPI(eventText, context) {
    ...
    parseSeverity(sev) {
        ...
        if (s === 'warning' || s === 'success' || s === 'info' || s === 'error')
            return s;
        if (s === 'critical')
            return 'error';
...
exports.GroqFallbackDetector = GroqFallbackDetector;
exports.default = GroqFallbackDetector;
//# sourceMappingURL=GroqFallbackDetector.js.map