...
export class DetectorManager {
  ...
  async runDetectorsOn(eventGroups: EventGroupMap): Promise<Finding[]> {
    const attemptedDetectors = new Set<Detector>()
    ...
    const groupDetectionTasks = Object.values(eventGroups).map(async (group: EventGroup) => {
      ...
      if (!groupFindings.length) {
        groupFindings = await this.runTier([this.analyzer], group, attemptedDetectors, false)
      }

      // tier 3 — llm fallback, only if analyzer found nothing
      if (!groupFindings.length) {
        groupFindings = await this.runTier([this.llmFallback], group, attemptedDetectors, true)
      }
    ...
    for (const detector of [...this.postProcessors, ...attemptedDetectors]) {
